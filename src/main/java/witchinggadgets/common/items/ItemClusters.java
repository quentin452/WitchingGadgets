...
public class ItemClusters extends Item {
    ...
    public static ItemStack getCluster(String ore) {
        if (WGConfig.allowClusters) for (int sn = 0; sn < subNames.length; sn++)
    ...
    public void getSubItems(Item item, CreativeTabs tab, List itemList) {
        if (WGConfig.allowClusters) for (int iOre = 0; iOre < subNames.length; iOre++)
    ...
    public static void setupClusters() {
        if (WGConfig.allowClusters) for (String ore : subNames)
            if (!OreDictionary.getOres("ore" + ore).isEmpty() && !OreDictionary.getOres("ingot" + ore).isEmpty()) {
                try {
                    ...
                    if (!colList.isEmpty()) {
                        ...
                        int clustertype = rgb[0] > rgb[2] && rgb[1] > rgb[2] ? 2
                                : rgb[0] > rgb[1] && rgb[0] > rgb[2] ? 1 : 0;
                        List<Integer> colours = ClientUtilities
                                .getItemColours(OreDictionary.getOres("ingot" + ore).get(0));
