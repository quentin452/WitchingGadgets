...
public class ItemPrimordialArmor extends ItemFortressArmor
        implements IActiveAbility, IPrimordialCrafting, IEventGear, IPrimordialGear {
    ...
    public void onArmorTick(World world, EntityPlayer player, ItemStack stack) {
        ...
        if (this.armorType == 3) {
            ...
            if (player.fallDistance > 0.0F) player.fallDistance -= 0.25F;
        }
                    ...
                    if (event.source.getSourceOfDamage() instanceof EntityLivingBase)
                        if (event.entityLiving.getRNG().nextInt(4) == 0) {
                            ((EntityLivingBase) event.source.getSourceOfDamage())
                                    .addPotionEffect(new PotionEffect(Potion.blindness.id, 10, 0));
                            ((EntityLivingBase) event.source.getSourceOfDamage())
                                    .addPotionEffect(new PotionEffect(Potion.moveSlowdown.id, 10, 3));
                        }
                    break;
                // case FIRE:
                // //Nova?
